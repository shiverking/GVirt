...
public:
                                ...
                                uint32_t batch, uint32_t indexTopK, float scale)
    {
        ...
        svk0 = aicHelper.Init(nHeads, ropeHeadDim, kvLoraRank, 0, qkStride, true);
    }
    ...
    __aicore__ inline void RunAic()
    {
        ...
        for (int batchIdx = 0; batchIdx < batch; batchIdx++) {
            ...
            for (int idx = firstCore; idx < taskNum; idx += GetBlockNum()) {
                ...
                aicHelper.RunAicQK(qAbsorb[absorbOffset], qr[qrOffset], kCache, peCache,
                                   queryTaskLen, nullptr, 0, calcLen, qk[curr]);
                ...
                if (needDoSV != 0) {
                    ...
                    aicHelper.RunAicSV(qk[last], lastKCache, lastQueryTaskLen, nullptr, 0,
        ...
        if (needDoSV != 0) {
            ...
            dbg_printf("block%d: {batch %d, query [%u - %u), headIdx [0 - %u), calcLen %d}"
                       ...
                       GetBlockIdx(), lastBatchIdx, lastQueryTaskOffset,
                       lastQueryTaskOffset + lastQueryTaskLen, nHeads, lastCalcLen, last);
            aicHelper.RunAicSV(qk[last], lastKCache, lastQueryTaskLen, nullptr, 0, lastCalcLen,
