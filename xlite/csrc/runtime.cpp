...
void XRuntime::SetMatmulBackend310P(const std::string &backend)
{
    ...
    if (backend == "ascendc_asr") {
        _matmulBackend310P = XMatmulBackend310P::ASCENDC_ASR;
        return;
    }
    ...
    throw std::invalid_argument(
        "Ascend310P matmul backend must be one of: ascendc_asr_nz, "
        "ascendc_asr_perf, ascendc_asr, "
...
const char *XRuntime::MatmulBackend310PName(void) const
{
    ...
    if (_matmulBackend310P == XMatmulBackend310P::ASCENDC_ASR) {
        return "ascendc_asr";
    }
    if (_matmulBackend310P == XMatmulBackend310P::ASCENDC_ASR_PERF) {
        return "ascendc_asr_perf";
    }
    if (_matmulBackend310P == XMatmulBackend310P::ASCENDC_ASR_NZ) {
        ...
        case 1: {
            ...
            if (UseDirectAtbDecodeAttention310P() &&
                ...
                CHECK_ACL(aclrtMemcpyAsync(
                    ...
                    ACL_MEMCPY_HOST_TO_DEVICE, stream));
                RecordDirectAtbMetadataH2D(offsetsBytes + lensBytes + tablesBytes, 3);
