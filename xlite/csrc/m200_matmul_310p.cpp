...
constexpr uint32_t kMaxAsrBatchedTokens = 4096;
                                  ...
                                  bool transpose, bool enablePrefill)
{
    ...
    const uint32_t maxM = enablePrefill ? kMaxAsrBatchedTokens : kMaxDecodeBatch;
    return m >= 1 && m <= maxM && decoderProjection && weight.shape[1] == k &&
