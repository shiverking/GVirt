...
build_dir=${XLITE_ASR_ATTN_CHAIN2_BUILD_DIR:-/tmp/xlite_asr_attention_partition_chain_release}
cann_path=${1:-${ASCEND_CANN_PACKAGE_PATH:-/usr/local/Ascend/cann-9.1.0-beta.1}}
case_filter=${2:-all}
jobs=${XLITE_BUILD_JOBS:-8}
warmup=${XLITE_ASR_ATTN_CHAIN2_WARMUP:-1}
iterations=${XLITE_ASR_ATTN_CHAIN2_ITERATIONS:-3}
...
if [[ ${executed} -eq 0 ]]; then
    echo "Unknown partition-chain case: ${case_filter}" >&2
    exit 2
fi
echo "ASR attention partition chain summary: $((executed-${#failures[@]})) passed, ${#failures[@]} failed"
