/*
 * Ascend310P3 Qwen3-ASR device-side paged-attention partition-state probe.
 * One M200 AI Core owns (request, query-head, partition).  A partition spans
 * at most 512 KV tokens and emits {max, sum, unnormalised_output[128]} in an
 * aligned 144-float record for the separately validated stable merge kernel.
 */
...
class AsrAttentionPartitionStateProbe {
public:
...
extern "C" __global__ __aicore__ void asr_attention_partition_state_probe(
    ...
    GM_ADDR kvLengths, GM_ADDR states, uint32_t batch, uint32_t tableStride)
{
